...
#pragma warning restore CA1859 // 盡可能使用具象類型以提高效能
    ...
    public override async Task UpdateVideosDataAsync(Channel channel, CancellationToken cancellation = default)
    {
        ...
        (bool isLive, string? videoId) = await GetFc2LiveStatusAsync(channel, cancellation);
        ...
        if (!string.IsNullOrEmpty(videoId))
        {
            ...
            FC2MemberData? info = await GetFc2InfoDataAsync(channel.id, cancellation);
            if (null == info) return;
                ...
                && info.Data.ChannelData.IsPremium == 0)
            {
                ...
                {
                    await fC2LiveDLService.CreateJobAsync(video: video,
...
#pragma warning restore CA1869
    ...
    public override async Task UpdateChannelDataAsync(Channel channel, CancellationToken stoppingToken)
    {
        ...
        FC2MemberData? info = await GetFc2InfoDataAsync(channel.id, stoppingToken);
        if (null == info)
        {
